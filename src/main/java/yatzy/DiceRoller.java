...
public class DiceRoller {
	...
	public boolean isYatzy() {
		return getCountsMap().entrySet().stream().anyMatch(e -> e.getValue() == 5);
	}
	...
	public Integer getNumberOfKind(int kindNumber) {
		return getCountsMap().entrySet().stream().filter(e -> e.getValue() >= kindNumber).map(Map.Entry::getKey)
				.max(Comparator.naturalOrder()).map(d -> d * kindNumber).orElse(0);
	}

	public boolean isSmallStraight() {
		return dices.stream().sorted().collect(toList()).equals(SMALL_STRAIGHT_LIST);
	}

	public boolean isLargeStraight() {
		return dices.stream().sorted().collect(toList()).equals(LARGEST_STRAIGHT_LIST);
	}
}